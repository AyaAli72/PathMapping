...
class RRTStar(Node):
    def __init__(self, start_location, goal_location):
        ...
        self.start = NodeStruct(start_location[0], start_location[1])
        self.goal = NodeStruct(goal_location[0], goal_location[1])
        self.node_list = [self.start]

        self.create_subscription(Pose2D, '/robot_pose', self.gps_callback, 10)
        self.create_subscription(LaserScan, '/scan', self.process_lidar_data, 10)
...
def main(args=None):
    ...
    goal_location = [start_location[0] + 5.0, start_location[1] + 20.0]
    node = RRTStar(start_location=start_location, goal_location=goal_location)
    rclpy.spin(node)
    rclpy.shutdown()
